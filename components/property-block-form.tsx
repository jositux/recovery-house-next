...
export function PropertyBlockForm() {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 rounded-lg border p-4 m4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between">
              <div className="space-y-0.5">
                <FormLabel className="text-base">Bloquear propiedad</FormLabel>
                <FormDescription>Activa esta opción y elihe un rango de fechas.</FormDescription>
              </div>
              <FormControl>
                <Switch
                  checked={field.value}
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value ? format(field.value, "PPP") : <span>Desde</span>}
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        disabled={(date) => {
                          ...
                          return date < new Date() || (toDate ? date > toDate : false)
                        }}
                        initialFocus
                      />
                    </PopoverContent>
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          ...
                          className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}
                        >
                          {field.value ? format(field.value, "PPP") : <span>Hasta</span>}
                          <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                        </Button>
                      </FormControl>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        disabled={(date) => {
                          ...
                          return date < new Date() || (fromDate ? date < fromDate : false)
                        }}
                        initialFocus
                      />
                    </PopoverContent>
