...
export default function BookingCompartidaAirbnb({
  ...
  const handleSingleBedsChange = (inc: number) =>
    setSingleBeds((prev) => Math.max(0, Math.min(prev + inc, availableSingleBeds)))

  const handleDoubleBedsChange = (inc: number) =>
    setDoubleBeds((prev) => Math.max(0, Math.min(prev + inc, availableDoubleBeds)))
  ...
  return (
    <div className="border rounded-lg p-4 sm:p-6 space-y-4 sm:space-y-6 shadow-md bg-white max-w-md mx-auto">
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
              ...
              disabled={(d) =>
                d <= (checkIn || today) || reservedLookup(d) || (checkIn ? hasBetween(checkIn, d) : false)
              }
      ...
      <div className="space-y-4 mt-6">
        <h3 className="font-medium text-[#162F40] mb-2">Selecciona tus camas:</h3>
