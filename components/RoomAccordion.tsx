...
const RoomAccordion = ({ rooms = [], setRooms }: RoomAccordionProps) => {
  ...
  const handleTagsChange = (tags: string[], roomId: string) => {
    updateRoomsState((prevRooms) =>
      prevRooms.map((room) =>
        room.id === roomId ? { ...room, extraTags: tags } : room
      )
    );
  };
  ...
  const handleServiceTagsChange = (tags: string[], roomId: string) => {
    updateRoomsState((prevRooms) =>
      prevRooms.map((room) =>
        room.id === roomId ? { ...room, servicesTags: tags } : room
      )
    );
  };
  ...
  const updateRoom = <T extends keyof Room>(
    id: string,
    field: T,
    value: Room[T]
  ) => {
    setRooms((prevRooms) =>
      prevRooms.map((room) => {
        if (room.id === id) {
          ...
          if (
            (field === "pricePerNight" || field === "cleaningFee") &&
            typeof value === "string"
          ) {
            const numericValue = parseFloat(value) || 0; // Convertir a número o usar 0 si no es válido
            adjustedValue = (
  ...
  const handleSelectChange = (
    ...
    id: string,
    field: keyof Room
  ) => updateRoom(id, field, parseInt(e.target.value));
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              ...
                              value={room.name}
                              onChange={(e) =>
                                handleInputChange(e, room.id, "name")
                              }
                            />
                          </div>
                          <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <label className="block mb-1 text-sm">Camas</label>
                            <select
                              value={room.beds}
                              onChange={(e) =>
                                handleSelectChange(e, room.id, "beds")
                              }
                          ...
                          <div>
                            ...
                            <select
                              value={room.capacity}
                              onChange={(e) =>
                                handleSelectChange(e, room.id, "capacity")
                              }
                              className="w-full p-2 border rounded"
                            >
                              {[1, 2, 3, 4, 5].map((cap) => (
