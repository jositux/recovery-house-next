...
import { getPropertiesByUserId, Property } from "@/services/propertyCollectionService"
...
const PropertiesPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      } catch (err: unknown) {  // Type the error correctly
            ...
            <Card key={property.id} className="flex flex-col">
              <CardHeader className="p-0">
                <div className="relative h-48 w-full">
                  <Image
                    src={`/webapi/assets/${property.mainImage || "/placeholder.svg"}`}
                    alt={property.name}
                    layout="fill"
                    objectFit="cover"
              ...
              <CardContent className="flex-grow p-4">
                ...
                <p className="text-sm text-gray-600 mb-2">País: {property.country}</p>
