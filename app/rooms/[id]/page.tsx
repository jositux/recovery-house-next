...
export default function RoomPage() {
      ...
      try {
        const [propertyResponse, bookingsResponse, providerResponse] = await Promise.all([
          axios.get("/webapi/items/Property", {
            params: {
              fields: "*,photos.directus_files_id.*, Rooms.*, Rooms.photos.*, Rooms.extraTags.*, Rooms.servicesTags.*",
            },
            headers: {
  ...
  useEffect(() => {
    if (room && room.photos) {
      setPhotoIds(room.photos.map((photo) => `/webapi/assets/${photo.directus_files_id}`))
    }
  }, [room])
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <PhotoGallery photos={photoIds} isOpen={isGalleryOpen} onClose={() => setIsGalleryOpen(false)} />
