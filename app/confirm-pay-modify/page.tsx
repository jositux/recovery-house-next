...
export default function NewConfirmAndPay() {
  ...
  // 🔹 Helpers
  const formatCurrency = (value: number) => value.toLocaleString("es-CO", { style: "currency", currency: "COP" })
  ...
  const getCurrentAmount = (b: BookingData) => {
    const previousBalance = Number(b.prepaymentAmount) || 0
    const amount =
      selectedDiscountOption === "with-discount" ? getPrepaymentAmount(b) : getBaseAmount(b) - previousBalance
