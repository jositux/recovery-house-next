...
// Importa los estilos de la ubicación original
...
export function RoomsPageClient({ lang = "es" }: { lang?: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (
          JSON.stringify(propertiesResponse.data.data) !==
          JSON.stringify(prevPropertiesRef.current)
        ) {
  ...
  const allRooms = useMemo(() => {
    return properties.flatMap((property) =>
      property.Rooms.map((room) => ({
        ...
        patology: property.patology
          ? property.patology.split(",").map((p) => p.trim())
          : [],
      }))
    );
  }, [properties]);
    ...
    () =>
      (
        ...
        return !bookings.some((booking) => {
          ...
          return (
            (requestedCheckIn >= bookingCheckIn &&
              requestedCheckIn < bookingCheckOut) ||
            (requestedCheckOut > bookingCheckIn &&
              requestedCheckOut <= bookingCheckOut) ||
            (requestedCheckIn <= bookingCheckIn &&
              requestedCheckOut >= bookingCheckOut)
          );
        });
  ...
  const filteredRooms = useMemo(() => {
    // Lee los parámetros de búsqueda directamente en el componente cliente
    const procedures = searchParams?.get("procedures")?.split(",") || [];
    const location = searchParams?.get("location") || "";
    const checkIn = searchParams?.get("checkIn") || "";
    const checkOut = searchParams?.get("checkOut") || "";
    const travelers = Number.parseInt(
    ...
    return allRooms.filter((room) => {
      const property = properties.find((p) =>
        p.Rooms.some((r) => r.id === room.id)
      );

      // Debug: Check taxIdApproved
      if (!property || !property.taxIdApproved) {
        console.log(
          `Room ${room.id} filtered out - No property or taxIdApproved:`,
          {
      ...
      const matchesProcedures =
        procedures.length === 0 ||
        procedures.some((proc) =>
          room.patology.some((pat) =>
            pat.toLowerCase().includes(proc.toLowerCase())
          )
        );
      ...
      const matchesLocation = location
        ? location
            ...
            .every(
              (word) =>
                normalizeString(room.propertyLocation.city).includes(
                  normalizeString(word)
                ) ||
                normalizeString(room.propertyLocation.state).includes(
                  normalizeString(word)
                ) ||
                normalizeString(room.propertyLocation.country).includes(
                  normalizeString(word)
                )
            )
        : true;
      ...
      const hasEnoughCapacity =
        room.isPrivate === false
          ? true
          : room.capacity === undefined || room.capacity >= travelers;
      ...
      if (!hasEnoughCapacity) {
        console.log(`Room ${room.id} filtered out - Not enough capacity:`, {
          ...
          note:
            room.isPrivate === false
              ? "Shared room - capacity check skipped"
              : "Private room - capacity check applied",
        });
        return false;
      }
      ...
      if (!isAvailable) {
        console.log(`Room ${room.id} filtered out - Not available:`, {
          ...
          checkOut,
          disabledDates,
          hasBookingConflict: bookings.some(
  ...
  useEffect(() => {
    ...
    setVisibleRooms((prevVisibleRooms) => {
      const roomsToRemove = prevVisibleRooms.filter(
        (id) => !newVisibleRooms.includes(id)
      );
      ...
      >
        <div
          className={`transition-all duration-300 ease-in-out ${
            isMapVisible
              ? "w-full lg:w-2/3 hidden lg:block"
              : "w-full pr-0 lg:pr-8"
          }`}
        >
          ...
          >
            {currentRooms.map((room, index) => {
              ...
              const isSearchActive = Object.values(searchParamsObj).some(v => v);

              const roomId = isSearchActive
                  ? `${room.id}?${new URLSearchParams(searchParamsObj).toString()}`
                  : room.id;
                    ...
                    <RoomCard
                      ...
                      price={
                        room.privateRoomPrice
                          ? Number.parseFloat(room.privateRoomPrice)
                          : 0
                      }
                      image={imageSrc}
                      propertyName={room.propertyName || ""}
        ...
        <div
          className={`transition-all duration-300 ease-in-out ${
            isMapVisible
              ? "w-full lg:w-1/3 opacity-100 visible"
        ...
        {isMapVisible ? (
          <>
            ...
            <span className="text-sm font-medium">
              {isSpanish ? "Mostrar habitaciones" : "Show Rooms"}
            </span>
          </>
        ) : (
          <>
            ...
            <span className="text-sm font-medium">
              {isSpanish ? "Mostrar Mapa" : "Show Map"}
            </span>
          </>
        )}
      </button>
