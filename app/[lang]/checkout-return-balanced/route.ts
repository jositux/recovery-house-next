...
export const GET = async (request: NextRequest) => {
  ...
  if (!stripeSessionId?.length) return redirect("/checkout-balanced");
  ...
  if (session.status === "complete") {
    ...
    if (
      ...
      return redirect(`/checkout-balanced/success/?rel=${paymentIntent.id}`);
  ...
  if (session.status === "open") {
    ...
    return redirect(`/checkout-balanced`);
  }
