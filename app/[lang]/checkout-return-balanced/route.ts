...
export const GET = async (request: NextRequest) => {
  ...
  // El idioma viene en la propia ruta (/es/checkout-return-balanced, /en/...);
  ...
  if (!stripeSessionId?.length) return redirect(`/${lang}/checkout-balanced`);
  ...
  if (session.status === "complete") {
    ...
    if (paymentIntentId) {
      return redirect(`/${lang}/checkout-balanced/success/?rel=${paymentIntentId}`);
    }
  }
  ...
  if (session.status === "open") {
    ...
    return redirect(`/${lang}/checkout-balanced`);
  }
