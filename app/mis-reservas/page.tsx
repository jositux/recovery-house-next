...
const BookingList: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const propertiesResponse = await fetch(
          `/webapi/items/Property?filter[userId][_eq]=${user.id}&fields=*,+Rooms.*`,
        )
  ...
  if (isLoading) {
    return <div className="container mx-auto p-4 text-center">Cargando reservas...</div>
  }
                    ...
                    <CardContent className="flex-1 p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="font-medium">${roomDetails?.pricePerNight}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-500">Limpieza</p>
                          <p className="font-medium">${roomDetails?.cleaningFee}</p>
                        </div>
                      </div>
                      {roomDetails && <p className="text-sm text-gray-600 mb-4">{roomDetails.description}</p>}
                      <div className="flex justify-between items-center">
                        <p className="text-lg font-semibold">Total: ${calculateTotal(booking, roomDetails)}</p>
                        <Button onClick={() => handleReviewClick(booking.id)}>Dejar Comentario</Button>
                      </div>
                    </CardContent>
                  </div>
                </Card>
