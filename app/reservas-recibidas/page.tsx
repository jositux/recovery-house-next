...
import { Calendar, Users, DollarSign, BedSingle, BedDouble, Star, Loader2, Search, MapPin, User, X } from "lucide-react"
  ...
  const handleConfirmCancel = async () => {
    if (selectedCancelBookingId && cancelReason.trim()) {
      try {
        ...
        // Here you would make the API call to cancel the booking
        await new Promise((resolve) => setTimeout(resolve, 500))

        // Remove the booking from the list
        setBookings((prev) => prev.filter((booking) => booking.id !== selectedCancelBookingId))
    ...
    const fetchData = async () => {
      try {
        ...
        const bookingsResponse = await fetch(
          `/webapi/items/Booking?filter[ownerId][_eq]=${user.id}&fields=*, +room.*, +room.photos.directus_files_id.id, +room.propertyId.*`,
        )
