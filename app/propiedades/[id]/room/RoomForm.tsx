...
export const formSchema = z
  .object({
    ...
    checkinTime: z
      .string()
      .min(1, { message: "Seleccione horario de check-in" }),
    checkoutTime: z
      .string()
      .min(1, { message: "Seleccione horario de check-out" }),
...
export default function RoomForm({ onSubmit, initialValues }: RoomFormProps) {
  ...
  const [checkinTime, setCheckinTime] = useState(
    initialValues?.checkinTime || "15:00"
  );
  const [checkoutTime, setCheckoutTime] = useState(
    initialValues?.checkoutTime || "11:00"
  );

  // State for discounts
  const [shortStayDiscount, setShortStayDiscount] = useState(
    initialValues?.shortStayDiscount || "0"
  );
  const [mediumStayDiscount, setMediumStayDiscount] = useState(
    initialValues?.mediumStayDiscount || "0"
  );
  const [longStayDiscount, setLongStayDiscount] = useState(
    initialValues?.longStayDiscount || "0"
  );
        ...
        <div className="space-y-4 p-4 bg-white rounded-xl">
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Establece los horarios estándar para la llegada y salida de los
              huéspedes.
            </p>
          ...
          <div className="p-3 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-600">
                Se permite al huésped pagar un <strong>anticipo del 10%</strong>{" "}
                para asegurar la reserva y completar el pago más adelante, con
                anulación gratuita en las primeras 24 horas. Para estadías
                largas (+10 noches), este anticipo es reembolsable bajo ciertas
                condiciones según las políticas de la plataforma Recovery Care
                Solutions.
              </p>
            </div>
          </div>
        ...
        <div className="flex gap-4 mt-4 p-4 md:p-0">
          ...
          >
            <Button
              variant="outline"
              type="button"
              className="w-full text-sm px-4 py-3 h-full bg-transparent"
            >
              Cancelar
            </Button>
          </Link>
